...
Toàn bộ gameplay/content hiện tại render bằng placeholder màu/hình khối thuần (`Phaser.GameObjects.Rectangle`/`Arc`/`Graphics` tint theo `tintColor`/`color` trong data, không có sprite/texture thật nào ngoài vài texture placeholder sinh trong `BootScene.generatePlaceholderTextures()`) — xem gotcha liên quan trong [ARCHITECTURE.md](../architecture/ARCHITECTURE.md) và [ASSET_GUIDE.md](../assets/ASSET_GUIDE.md). Trước khi bắt đầu tìm/tích hợp asset thật, cần audit lại đúng danh sách đầy đủ entity cần sprite (đếm thật từ data, không áng chừng):

- [ ] **5 nhân vật** — `data/characters.json` (Soul Hunter + 4 nhân vật unlock bằng Coin/Permanent Upgrade)
...
- [ ] **7 vũ khí** — 5 vũ khí cơ bản (Sword/Fireball/Ice Shard/Lightning/Boomerang) + 2 vũ khí đặc biệt khoá (Holy Javelin/Storm Hammer, xem SESSION_SUMMARY.md) trong `data/weapons.json`, cộng thêm **15 vũ khí Fusion** trong `data/fusionWeapons.json` (hiện dùng chung màu/hình khối vũ khí gốc, có thể cần hiệu ứng/tint riêng để phân biệt)
