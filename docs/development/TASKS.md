...
- [x] Map Phase B (module doc đầy đủ — xem [MAP.md](../gameplay/MAP.md)): điền nội dung thật cho 8 map còn lại (Swamp/Frozen Tundra/Volcanic/Desert Ruins/Corrupted Castle/Crystal Caves/Sky Sanctum/Void Abyss) — mỗi map 5 quái thường mới (`enemies_*.json`) + 1 boss mới (`bosses.json`, tái dùng đúng 5 skill có sẵn, không viết skill mới), `maps.json` đổi id/name/enemyDataFile/bossId khỏi placeholder Map3-10, giữ nguyên `order`/`difficultyMultiplier`/chuỗi `unlockRequires` tuần tự
...
- [x] 6 Boss Skill mới + Mid-Boss/map (module doc đầy đủ — xem [BOSSES.md](../gameplay/BOSSES.md) mục "Boss Skills" + "Mid-Boss + Boss cuối"): thêm Teleport/Meteor/Poison Cloud/Heal Self/Clone/Freeze Pulse vào kiến trúc plug-in `Boss.ts`/`BossSystem.ts` có sẵn (tái dùng cờ `pendingSlamDamage` cho Meteor, tái dùng `PoolManager.getEnemy()` cho Clone). Generalize `GameScene` để mỗi map từ Swamp trở đi có 2 boss (Mid-Boss + boss cuối) thay vì 1 — thêm `MapDef.midBossId`, `startBossIntro(bossId)` tổng quát. Thêm 8 Mid-Boss mới + cập nhật `skillIds` 8 boss cuối đã có (map3-10), đã đối chiếu không trùng tổ hợp skill trong 19 boss. Fix 1 race condition phát hiện lúc test (Mid-Boss chết sát player → Loot Chest + Boss Intro cùng pause chồng nhau trong 1 frame) bằng cờ `lootFlowActive`.
- [x] Fix BUG NGHIÊM TRỌNG: 14/21 upgrade Level Up không có tác dụng thật (module doc đầy đủ — xem [UPGRADE.md](../gameplay/UPGRADE.md) mục "Cơ chế đọc/áp dụng từng stat"). Nguyên nhân gốc: `UpgradeSystem.applyUpgrade()` set `player.stats[def.stat]` qua index signature nên KHÔNG BAO GIỜ báo lỗi dù không có code nào đọc lại field đó — nhiều stat (Move Speed, Crit Chance/Damage toàn bộ hệ thống, Projectile+1, Fireball Size, Sword Range, Poison, HP Regen, Cooldown Reduction, Dodge/Shield/Armor, Soul Value) đã được set từ trước nhưng thiếu hẳn code tiêu thụ; riêng Life Steal đọc nhầm field khác (`WeaponDef.lifeStealPercent` thay vì `player.stats.lifeStealPercent`). Đã implement code tiêu thụ thật cho toàn bộ 14 stat trong `Player.ts`/`WeaponSystem.ts`/`UpgradeSystem.ts`, verify từng cái qua console/gameplay thật (Sword Range — vòng chém to gấp 3, HP Regen — đo tăng dần theo thời gian thực, Shield/Dodge/Armor — gọi `takeDamage()` trực tiếp đối chiếu số).
...
## Sprite nhân vật động theo characterId + vũ khí Bow/Triple Throw

- [x] **Load sprite động theo characterId** (module doc đầy đủ ở [ARCHITECTURE.md](../architecture/ARCHITECTURE.md) mục "Sprite nhân vật động theo characterId"): `utils/CharacterSpriteLoader.ts` load Run/Attack/projectile riêng theo `characterId` (không hardcode 1 nhân vật cố định như trước), texture key namespace `run_<characterId>_<dir>`/`attack_<characterId>_<dir>` để nhiều nhân vật cùng tồn tại song song không đè nhau khi đổi nhân vật giữa phiên. `BootScene.preload()` load nhân vật đang chọn lúc khởi động, `GameScene.preload()` (qua `init(data)`) tự load lại nếu đổi nhân vật — dùng đúng Phaser load queue (`this.load.spritesheet()` trong `preload()`), Phaser tự đảm bảo `create()` chỉ chạy sau khi load complete, không cần code chờ thủ công. Bảng map characterId → thư mục asset khai báo TƯỜNG MINH (không suy diễn bằng string template) vì tên thư mục thật không khớp characterId ở nhiều chỗ (`hunter`→`archer/`, `assassin`→`assasin/` thiếu 1 chữ s...).
- [x] **Fallback khi thiếu Run/Attack** — không crash, vũ khí vẫn bắn dù thiếu animation: thiếu Run ưu tiên đứng yên ở frame đầu Attack (nếu có) thay vì `player_placeholder`; thiếu Attack thì `playAttackAnimation()` vẫn gọi `onRelease()` ngay dù không có animation để chạy.
- [x] **Fix bug chớp nháy animation khi đứng yên** (Assassin — thiếu Run, có Attack tự bắn theo cooldown): nguyên nhân KHÔNG phải gọi lại `anims.play()` mỗi frame (2 nhánh code đều đã guard sẵn) mà là oscillation theo chu kỳ cooldown — mỗi lần Attack1 kết thúc, code cũ luôn rơi về `player_placeholder` làm Idle tạm, rồi bắn tiếp hiện lại hình thật, lặp lại liên tục. Fix bằng `lastDirection`/`lastMovementState` (chỉ set lại animation khi 1 trong 2 giá trị thực sự đổi) + đổi fallback ưu tiên frame đầu Attack thay vì rơi thẳng về placeholder — verify deterministic (10 lần gọi liên tiếp state không đổi → chỉ 1 lần set texture, không phải 10).
- [x] **Bow (Hunter, thay Sword)** — đồng bộ frame nhả dây cung với projectile thật (module doc đầy đủ ở [WEAPONS.md](../gameplay/WEAPONS.md)): mũi tên thật chỉ bắn đúng lúc animation Attack1 chạy tới frame 3 (xác định bằng cách xem trực tiếp từng frame PNG, không đoán), dùng sự kiện `animationupdate` chung của Phaser (không có biến thể theo key như `animationcomplete-<key>`, đã verify trong source Phaser). Animation hiển thị theo `aimAngleRad` quy đổi 4 hướng, quỹ đạo mũi tên thật vẫn bay đúng góc atan2 chính xác, tách biệt hoàn toàn.
- [x] **Triple Throw (Assassin, thay Boomerang)** — 3 dao bắn cùng lúc tỏa hình quạt ±27° quanh hướng quái gần nhất (`findNearestTarget()`, KHÔNG dùng `player.currentDirection`), `type: "projectile_straight"` — trúng quái đầu tiên hoặc hết `maxDistance` (480, riêng cho vũ khí này) thì despawn, ĐÃ BỎ hẳn cơ chế bay-xa-rồi-quay-lại player (bản đầu dùng type `triple_spread_return` tái dùng cơ chế Boomerang, sau đổi thiết kế bỏ hẳn theo yêu cầu). Mỗi dao `setAngle()` riêng theo góc bay của chính nó.
- [x] Ảnh mũi tên/dao thật (`arrow_projectile.png`, `assasin_projectile.png`) thay placeholder Graphics vẽ tay ban đầu — origin neo giữa (0.5, 0.5) để xoay đúng tâm.

## Chuẩn bị chuyển sang giai đoạn Art

Phần lớn gameplay/content vẫn render bằng placeholder màu/hình khối thuần (`Phaser.GameObjects.Rectangle`/`Arc`/`Graphics` tint theo `tintColor`/`color` trong data) — xem gotcha liên quan trong [ARCHITECTURE.md](../architecture/ARCHITECTURE.md) và trạng thái asset thật hiện có ở [ASSET_GUIDE.md](../assets/ASSET_GUIDE.md). Sprite NHÂN VẬT đã bắt đầu có art thật (mục trên) — phần còn lại (quái/boss/vũ khí gốc/UI icon) vẫn placeholder. Trước khi tìm/tích hợp thêm asset thật, cần audit lại đúng danh sách đầy đủ entity còn thiếu sprite (đếm thật từ data, không áng chừng):

- [x] **5 nhân vật** — Hunter/Knight/Assassin đủ art (Assassin thiếu riêng Run), Mage chỉ có Attack, Archer (roster riêng) chưa có gì — xem [ASSET_GUIDE.md](../assets/ASSET_GUIDE.md) bảng chi tiết
- [ ] **50 loại quái thường** — 10 map × 5 quái/map (`data/enemies*.json`: `enemies.json` + `enemies_graveyard.json` + 8 file map3-10 mới thêm ở sprint Map Phase B), mỗi map 1 bộ màu/tông riêng theo `tintColor`
- [ ] **19 boss** — `data/bosses.json` (10 map × Mid-Boss + boss cuối, trừ Forest/Graveyard chỉ 1 boss/map, + Orc Warlord chưa wire map)
- [ ] **9 vũ khí** — Sword/Bow/Fireball/Ice Shard/Lightning/Boomerang/Triple Throw + 2 vũ khí đặc biệt khoá (Holy Javelin/Storm Hammer, xem SESSION_SUMMARY.md) trong `data/weapons.json` — Bow/Triple Throw đã có ảnh projectile thật, 7 vũ khí còn lại vẫn placeholder Graphics; cộng thêm **15 vũ khí Fusion** trong `data/fusionWeapons.json` (hiện dùng chung màu/hình khối vũ khí gốc, có thể cần hiệu ứng/tint riêng để phân biệt)
- [ ] **2 Pickup** — Heal Potion, Magnet Orb (`data/pickups.json`)
- [ ] **UI icon** — icon vũ khí (`ui/WeaponIcon.ts`), icon Card/Upgrade (`ui/CollectionCard.ts`), icon vector Thử Thách 7 Ngày (`ui/ChallengeIcons.ts`), icon Pause/Collection/Điểm danh ở Menu — hiện toàn bộ vẽ bằng Graphics/text ký hiệu, chưa có icon hình ảnh thật

Không tự bắt đầu tìm/tải asset khi chưa có audit đầy đủ + xác nhận phong cách hình ảnh (pixel art/vector/flat...) với user — xem [ASSET_GUIDE.md](../assets/ASSET_GUIDE.md).
